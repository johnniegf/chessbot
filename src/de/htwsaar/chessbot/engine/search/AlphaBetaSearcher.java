 ...
 */
public class AlphaBetaSearcher extends Thread //implements MoveSearcher 
{
    ...
    private boolean checkTimeOut() {
        return mLimits.depth != 0 && mCurrentSearch.depth > mLimits.depth
                || mLimits.nodes != 0 && mCurrentSearch.nodes > mLimits.nodes
                || mLimits.msec != 0 && mCurrentSearch.msec > mLimits.msec;
    }
    ...
    private int search(final Board board,
         ...
         * TIMEOUT CHECK
         ***********************************************/
        if (!isSearchStopped() && checkTimeOut()) {
            stopSearch();
        }
        if (isSearchStopped()) {
            return 0;
